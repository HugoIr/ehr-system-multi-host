...
COMPOSE_FILE_BASE=docker/docker-compose-net-peer1hospital.yaml
...
function deployCC() {
  docker exec cli scripts/deployCC-host2.sh $CHANNEL_NAME $CC_NAME $CC_SRC_PATH $CC_SRC_LANGUAGE $CC_VERSION $CC_SEQUENCE $CC_INIT_FCN $CC_END_POLICY $CC_COLL_CONFIG $CLI_DELAY $MAX_RETRY $VERBOSE
...
function createChannel() {
  ...
  scripts/channel-peer1hospital.sh $CHANNEL_NAME $CLI_DELAY $MAX_RETRY $VERBOSE
}
